...
- **Location matching** is place-aware since 2026-09 (docs/place-search.md): gazetteer + listing classification + nearest fallback, one aggregate + one card read (≤ 3 ops with dates); `GET /places/index` and `countstays` share the edge cache and the `listings` tag.
