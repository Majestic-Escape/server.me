...
| `GET /prop-listing/?status=` | active only | active only | own list: any status | any status | 2026-09 (place search): anonymous `?status=incomplete` (and the default `$ne:"incomplete"`) listed drafts, listings in review and delisted ones — now `active` for everyone but an admin (DB-verified actor) or the host listing their own (`hostEmail` = self). Non-string / operator-shaped params and unknown statuses → 400; `searchTerm` is literal (escaped, ≤ 100 chars) — it was a raw regex (ReDoS / match-all). `tests/batch-s/listing-exposure.test.js` |
| `GET /property-registration-no/:n` | 200 / 404 | 200 / 404 | 200 / 404 | 200 / 404 | literal case-insensitive match (escaped, ≤ 64 chars): `.*` used to "exist" and pass the Goa registration check |
| `GET /property-registration-no/`, `POST /property-registration-no/` | 401 | 403 | 403 | 200 | 2026-09: were anonymous — the whole registry (~1.6 MB) per call, and anyone could insert a fake Goa number; no client calls them (seeding is `loadData.js`) |
